...
const GoogleCalendarMonthly: React.FC<{ events: GoogleCalendarEventProps[] }> = ({ events }) => {
    // Use state to manage the currently displayed month
    const initialDays: Date[] = events.map((event) => new Date(event.start.dateTime));
    ...
    return(
        ...
        {events.map((event, index) => {
            return(
                <div key={index} className="text-white"> {event.summary}</div>
            )
        })}
        </div>
        </div>
